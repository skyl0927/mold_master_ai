...
| 4 | Contract suite includes the structured output schema regression | `scripts/run-contract-tests.js` | Contract | PASS | `npm run test:contracts` |
...
The OpenAI SDK installed in this workspace exposes `response_format` support for `json_schema`; the implementation replaces Vision-only `json_object` mode with a strict schema. The report-generation JSON call remains unchanged because this TDD task only covers blind Vision observation.
...
This tightens the OpenAI Vision response contract. Gemini still receives a strongly worded JSON prompt and MIME type, but a provider-specific Gemini schema migration remains a separate follow-up.
