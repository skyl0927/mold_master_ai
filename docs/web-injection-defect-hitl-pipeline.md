...
- 기준 수집 폴더: `artifacts/web-injection-defect-cases-20260724T045605`
- 후보 카드: 40건
- 정규화 결함 클래스: 22개
- 기술 문헌 카드: 24건
- 라이선스 이미지 카드: 16건
...
- 카드 스키마 오류: 0건
- 이미지 SHA-256 불일치: 0건
- Common Agent 비저장 단건 검증: 40/40 통과
...
- Wikimedia Commons의 16개 시각 자료는 파일 페이지에서 저자, 라이선스, 원본 URL을 확인하고 로컬 사본 해시를 저장한다.
...
2. `Web Case HITL (40)` 탭을 선택한다.
3. 결함명, 현상, 원인, 출처와 이미지 라이선스를 확인한다.
4. 원인 라벨, 확인 항목, 대책, 검토자와 검토 의견을 입력한다.
5. 명시적 확인 체크 후 `HITL 승인`, `수정 필요`, `반려` 중 하나를 저장한다.
6. 승인 카드에서 `Common Agent 비저장 검증`을 실행한다.
7. 품질 게이트 통과 후 `후보 적재`를 실행한다.
8. 두 번째 확인 체크 후 `중앙 승인 + Graph 활성화`를 실행한다.
9. `Graph 왕복 검증`으로 `graph_approved_only` 답변과 승인 근거 경로를 확인한다.
