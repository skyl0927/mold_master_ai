...
Phase 1~6의 소프트웨어 구현과 자동 검증은 완료했다. 다음 사이클은 기능
추가보다 승인된 현장 데이터로 운영 합격 기준을 충족하는지 측정하는 단계다.

1. 핵심 결함군마다 최소 30개의 승인된 다중 시점 세션 확보
2. 제품군·금형·카메라·촬영 날짜 단위로 학습/holdout 분리 확정
3. 현재 운영 버전과 후보 버전을 같은 holdout 세트에서 shadow 실행
4. `npm run eval:vision:release`로 승격·유지·롤백 보고서 생성
5. 보고서를 앱의 비전 릴리스 게이트에 등록하고 HITL 담당자가 승인
6. 회귀 발생 시 보고서의 모델·프롬프트·Graph 직전 스냅샷으로 롤백
7. 월별 오류 코호트와 신규 제품군 30건 검증 결과로 다음 후보를 구성
