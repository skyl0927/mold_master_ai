...
Phase 1과 Phase 2의 소프트웨어 구현은 완료했다. 다음 개발 사이클은
Phase 3의 다중 시점 융합을 대상으로 한다.

1. 같은 `capture_session_id`의 모든 시점 이미지를 하나의 진단 실행에 전달
2. 시점별 Vision V2 관찰을 독립 생성하고 이미지·시점 계보를 보존
3. 결함 taxonomy 기준으로 후보를 병합하는 결정론적 융합기 구현
4. 전체, 근접, 사선광, 기능 위치별 관찰 가중치와 충돌 점수 정의
5. 추가 촬영, 사람 검토, 유력 후보의 세 가지 출구를 세션 단위로 판정
6. 승인 세션으로 클래스별 신뢰도 보정 및 위험 오판율 측정
7. GraphRAG에는 융합된 관찰과 모든 후보만 전달하고 원인·대책은 Phase 4에서 생성

실제 승인 사진으로 촬영 준비도 80%를 달성하기 전에는 Phase 3의 자동 확정
임계값을 완화하지 않는다.
