...
다음 개발 사이클은 Phase 1만 대상으로 한다.

1. `CapturedImage`에 촬영 세션, 시점 태그, 이미지 종류, 입력 출처 추가
2. 촬영 세션 순수 로직과 누락 시점 테스트를 먼저 작성
3. 외부 카메라를 다중 촬영 UI로 변경하고 촬영 후 창을 유지
4. 업로드·화면 캡처에도 동일 세션 메타데이터 적용
5. 전체·근접 사진이 없으면 진단 요청을 fail-closed 처리
6. Common Agent 요청에 촬영 세션과 시점 정보를 전달
7. 단위, 계약, TypeScript, 빌드, Electron smoke 검증

Phase 1 완료 후 실제 승인 사진으로 준비도 80%를 달성하기 전에는
Phase 3의 자동 확정 임계값을 완화하지 않는다.
