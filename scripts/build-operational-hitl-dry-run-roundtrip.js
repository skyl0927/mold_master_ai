...
const baseOutput = valueAfter('--output-base')
  || process.env.OPERATIONAL_HITL_DRY_RUN_ROUNDTRIP_OUTPUT_BASE
  || path.join(artifactRoot, `operational-hitl-dry-run-roundtrip-${timestamp()}`);

const jsonOutputPath = path.resolve(`${baseOutput}.json`);
const csvOutputPath = path.resolve(`${baseOutput}.simulation-only.csv`);
...
const run = () => {
  ...
  if (report.simulatedCsv) writeText(csvOutputPath, report.simulatedCsv);
  if (report.markdown) writeText(markdownOutputPath, report.markdown);
  writeJson(jsonOutputPath, {
    ...report,
    simulatedCsv: undefined,
    markdown: undefined,
    simulatedCsvPath: report.simulatedCsv ? csvOutputPath : null,
  ...
  console.log(JSON.stringify({
    ...
    simulatedRows: report.summary.simulatedRows,
  ...
  if (report.status === 'missing_evidence' || report.status === 'simulated_roundtrip_invalid') {
    process.exitCode = 1;
  }
};
